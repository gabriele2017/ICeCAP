...
getPairMat <- function(x, x.perc, rmMulti=0, rmSingle=0){
  ...
  if (rmMulti == 1 && rmSingle ==1){
    n.plots <- n.plots + 2
    notreported.lab <- c(notreported.lab, "Duplicate_pairs", "Pairs_with_singleton")
  }
        ...
        xlab(xlab) + ylab("Read Counts") +
            scale_x_discrete(breaks=c("1", "2"), labels=c("All Pairs","Filtered Pairs"))+
               geom_text(position=position_stack(vjust=0.5),aes(label=paste(perc,"%")),fontface="bold", size=4)+
                ggtitle("Statistics after Read Pairing") + theme(plot.title = element_text(lineheight=.8, face="bold", size=12))
...
mat$lab <- factor(mat$lab, levels=c("Unmapped_pairs", "Not_Reported_pairs", "Reported_pairs", "Low_qual_pairs",  "Pairs_with_singleton", "Duplicate_pairs"))
p1 <- plotPairStat(mat, xlab=sampleName)
ggsave(file.path(picDir, paste0("NGS_pairing_",sampleName,".pdf")), p1, width=12, height=10)
