...
getPairMat <- function(x, x.perc, rmMulti=0, rmSingle=0){
  ...
  n.plots <- 4
  notreported.lab <- c("Self_Ligations")
  ## Remove multi pairs
  if (rmMulti == 1 && rmSingle ==1){
    n.plots <- n.plots + 4
    notreported.lab <- c(notreported.lab, "Same_internal","Artefacts","Next_fragment","Religation","Next_fragment2")
  }
  ...
  stopifnot(x[reported.lab]+x[allnotreported.lab]==x["Total_pairs_processed"])
  ...
  p <- rep(1, n.plots)
  print(p)
  print(c(reported.lab, allnotreported.lab, notreported.lab))
  names(p) <- c(reported.lab, allnotreported.lab, notreported.lab)
  p[notreported.lab] <- 2
...
##
plotPairStat2 <- function(mat, xlab="", legend=TRUE){
  require(RColorBrewer)
  require(ggplot2)
  require(grid)

pie <- ggplot(mat, aes(x=p, as.numeric(count), fill=lab)) +
 geom_bar(width = 1)
...
plotPairStat <- function(mat, xlab="", legend=TRUE){
  ...
  gp <-ggplot(mat, aes(x=p, as.numeric(count), fill=lab)) +
    geom_bar(width=.7,stat="identity", colour="gray") + 
      theme(axis.title=element_text(face="bold", size=10), axis.ticks = element_blank(),  axis.text.y = element_text(size=8), axis.text.x = element_text(size=6))+
        xlab(xlab) + ylab("Read Counts") +
            scale_x_discrete(breaks=c("1", "2"), labels=c("All Pairs","Filtered Pairs"))+
               geom_text(position=position_stack(vjust=0.5),aes(label=paste(perc,"%")),fontface="bold", size=4)+
                ggtitle("Statistics after Bona Fide Filtering") + theme(plot.title = element_text(lineheight=.8, face="bold", size=12))
...
mat$lab <- factor(mat$lab, levels=c("Not_Reported_pairs", "Reported_pairs", "Self_Ligations",  "Same_internal", "Artefacts","Next_fragment","Religation","Next_fragment2"))
p1 <- plotPairStat(mat, xlab=sampleName)
ggsave(file.path(picDir, paste0("plotMappingPairing_",sampleName,".pdf")), p1, width=12, height=10)
