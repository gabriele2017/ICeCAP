...
for FILE in `ls $HM"/"frag_*_"$CHROM"_*_*.in|gawk '{print $NF}' FS="/"|sort -nk2,2 -t "_" `; do

cat $HM"/"$FILE >> $GRAPHICS"/HMRAW_"$CHROM
echo "" >> $GRAPHICS"/HMRAW_"$CHROM

for IN in {1 .. $INT}; do
gawk '{if(FNR==1){n=split(FILENAME,a,"_");if(NR>1){printf("\n")};printf("chr%s\t%s\t%s\t",a[n-2],a[n-1],a[n-1]+'$RES');};printf("%f\t",$5)}' $HM"/"$FILE |gawk '{if($1=="chr"'$CHROM'){print $0}}' >> $GRAPHICS"/RAWTADS_"$CHROM"_"$IN
done

done
CB=`gawk '{if($5!=0){c=c+1;a=a+$5;print a/c}}' $GRAPHICS"/HMRAW_"$CHROM"_*"|tail -n 1`
/usr/bin/gnuplot <<EOF

set xlabel "Enriched_Region_$CHROM"
set ylabel "Enriched_region_$CHROM"
set pm3d map
set size 0.8,1
...
for FILE in `ls $HM"/"frag_*_"$CHROM"_*_*.fin|gawk '{print $NF}' FS="/"|sort -nk2,2 -t "_" `; do
...
gawk '{if(FNR==1){n=split(FILENAME,a,"_");if(NR>1){printf("\n")};printf("chr%s\t%s\t%s\t",a[n-2],a[n-1],a[n-1]+'$RES');};printf("%f\t",$5)}' $HM"/"$FILE |gawk '{if($1=="chr"'$CHROM'){print $0}}' >> $GRAPHICS"/TADS_"$CHROM"_"$IN
