...
ploMapStat <- function(mat, sampleName="", tag="", legend=TRUE){
  ...
  if (legend){
    gp = gp + scale_fill_manual(values=c(sel.colours[2:4], "darkgray"), labels = c("Mapped Reads (%)",  "Unmapped (%)", "Digested reads (%)", "Non Digested Reads (%)")) + guides(fill=guide_legend(title="")) + theme(plot.margin=unit(x=c(1,0,0,0), units="cm"), legend.position="bottom", legend.margin=unit(.5,"cm"), legend.text=element_text(size=8))
  }else{
...
allmapstat_r1 <- list.files(path=bwtDir, pattern=paste0("^[[:print:]]*","read1","[[:print:]]*\\.mapstat$"), full.names=TRUE)

allmapstat_r2 <- list.files(path=bwtDir, pattern=paste0("^[[:print:]]*","read2","[[:print:]]*\\.mapstat$"), full.names=TRUE)
...
p2 <- ploMapStat(mat_r2, sampleName=sampleName, tag="R2")

pdf(file.path(picDir, paste0("NGS_mapping_",sampleName,".pdf")), width=14, height=8)
