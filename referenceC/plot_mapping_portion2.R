...
ploMapStat <- function(mat, sampleName="", tag="", legend=TRUE){
  ...
  tit <- "Statistics after Read Alignment"
  if (tag != ""){
  ...
  gp <- ggplot(mat, aes(x=p, as.numeric(count), fill=as.character(lab))) +
    geom_bar(width=.7,stat="identity", colour="gray") +
      theme(axis.title=element_text(face="bold", size=10), axis.ticks = element_blank(), axis.text.y = element_text(size=8), axis.text.x = element_blank()) +
          xlab(sampleName) + ylab("Read Counts")  +
                geom_text(position=position_stack(vjust=0.5),aes(label=paste(perc,"%")),fontface="bold", size=3)+
                ggtitle(tit) + theme(plot.title = element_text(lineheight=.8, face="bold", size=10))

  if (legend){
    gp = gp + scale_fill_manual(values=c(sel.colours[2:4], "darkgray"), labels = c("Full read mapping (%)",  "In silico digested Mapping (%)", "Aligned reads (%)", "Not aligned (%)")) + guides(fill=guide_legend(title="")) + theme(plot.margin=unit(x=c(1,0,0,0), units="cm"), legend.position="bottom", legend.margin=unit(.5,"cm"), legend.text=element_text(size=8))
  }else{
...
allmapstat_r1 <- list.files(path=bwtDir, pattern=paste0("^[[:print:]]*","cflashed","[[:print:]]*\\.mapstat$"), full.names=TRUE)

allmapstat_r2 <- list.files(path=bwtDir, pattern=paste0("^[[:print:]]*","nflashed","[[:print:]]*\\.mapstat$"), full.names=TRUE)
...
p1 <- ploMapStat(mat_r1, sampleName=sampleName, tag="Flashed Reads")
mat_r2 <- getMapMat(stats_per_sample_r2[1], stats_per_sample_r2[2], stats_per_sample_r2[3], stats_per_sample_r2[4])
p2 <- ploMapStat(mat_r2, sampleName=sampleName, tag="Non Flashed Reads")

pdf(file.path(picDir, paste0("NGS_flashedmapping_",sampleName,".pdf")), width=14, height=8)
