...
printf("ditag file: %s\n",concat(concat(concat(indir,".pairs."),record[chrom].name_chromosome),".dedup.sam"));
...
sprintf(stamp2,"_%s_%d_%d",outprefix,chrom_start,chrom_end);
      ...
      if(strlen(outdir)==0){mkdir(concat(folderwrite,lane),0777);
                            ...
                            /*outdir3=concat(concat(concat(concat(concat(concat(concat(folderwrite,lane),stamp),"/"),stats),"/"),lane),"");*/
                             outdir3=concat(concat(folderwrite,lane),"/data/");
...
cat $work_path$file.matefixed.sam | gawk '{if($9>0&&$12>0){print $0}}'>$work_path$file.matefixed.MQ30.sam
...
wc -l $out_path$file.pairs.chr*.dedup.sam >> $out_path$file.COUNTS.txt
...
Low_qual_pairs=`cat $work_path$file.matefixed.sam | gawk '{if($9<1||$12<1){print $0}}'|wc -l`

dedupl=`cat $out_path$file.pairs.chr*.dedup.sam|wc -l`
